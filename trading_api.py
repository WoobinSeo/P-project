...
from kis_broker import KISBroker, KISConfig, KISApiError
...
class AutoTradeRunDetail(BaseModel):
    id: int
    created_at: datetime
    returncode: int
    stdout: str
...
def _http_exception_from_kis_error(err: KISApiError) -> HTTPException:
    """
    KISApiError 를 FastAPI HTTPException 으로 변환.

    - 토큰 레이트 리밋(EGW00133), 모의투자 비영업일(40100000) 등은
      프론트가 이해하기 쉬운 한국어 메시지와 적절한 HTTP 상태 코드로 매핑한다.
    """
    body = err.body
    ...
    if isinstance(parsed, dict):
        error_code = parsed.get("error_code") or parsed.get("errorCode")
        error_description = parsed.get("error_description") or parsed.get("errorDescription")
    ...
    if error_code == "EGW00133":
        return HTTPException(
            status_code=429,
            detail="KIS 토큰 발급 제한: 1분에 1회만 토큰을 발급할 수 있습니다. 잠시 후 다시 시도해주세요.",
        )
    ...
    if msg_cd == "40100000":
        return HTTPException(
            status_code=400,
            detail="KIS 모의투자 영업일/영업시간이 아니라 주문이 거절되었습니다. 장 운영 시간에 다시 시도해주세요.",
        )

    # 3) 그 외 일반적인 KIS 도메인 에러
    if msg1 and msg_cd:
        msg = f"KIS 오류: {msg1} (코드: {msg_cd})"
    elif error_description and error_code:
        msg = f"KIS 오류: {error_description} (코드: {error_code})"
    else:
        msg = str(err)

    # 외부 연동 실패로 보고 502로 응답
...
def place_market_order(
    ...
    try:
        ...
        # KIS API 도메인 에러는 사용자 친화적인 HTTP 예외로 변환
        raise _http_exception_from_kis_error(e)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"KIS 주문 실패: {e}")
...
@app.get("/auto-trade/latest", response_model=AutoTradeRunDetail)
def get_auto_trade_latest():
    db = get_db()
    session = db.get_session()
    try:
        r = session.query(AutoTradeRun).order_by(AutoTradeRun.created_at.desc()).first()
    finally:
        session.close()

    if not r:
        raise HTTPException(status_code=404, detail="자동매매 실행 이력이 없습니다.")

    return AutoTradeRunDetail(
        id=r.id,
        created_at=r.created_at,
        returncode=r.returncode,
        stdout=r.stdout or "",
...
@app.get("/auto-trade/run/{run_id}", response_model=AutoTradeRunDetail)
def get_auto_trade_run(run_id: int):
    db = get_db()
    session = db.get_session()
    try:
        r = session.query(AutoTradeRun).filter(AutoTradeRun.id == run_id).first()
    finally:
        session.close()

    if not r:
        raise HTTPException(status_code=404, detail="해당 실행 이력을 찾을 수 없습니다.")
