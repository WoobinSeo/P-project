...
class KISApiError(Exception):
    """
    KIS REST 호출 실패를 표현하는 공통 예외.

    - status_code: HTTP 상태 코드
    - body: 파싱된 JSON(dict) 또는 원시 텍스트
    - phase: "token" / "order" / "balance" / "price" 등 컨텍스트
    """

    def __init__(self, status_code: int, body: Union[Dict[str, Any], str], phase: str = ""):
        self.status_code = status_code
        self.body = body
        self.phase = phase

        # 디버깅을 위해 기본 메시지에도 상태/본문을 포함
        super().__init__(f"KIS API 오류({phase}): status={status_code}, body={body}")
...
class KISBroker:
    ...
    def _get_access_token(self) -> str:
        ...
        if resp.status_code != 200:
            # 토큰 발급 오류는 KISApiError 로 래핑
            body: Union[Dict[str, Any], str] = js or resp.text
            raise KISApiError(status_code=resp.status_code, body=body, phase="token")

        access_token = js.get("access_token")
        if not access_token:
            raise KISApiError(
                status_code=resp.status_code,
                body=js or resp.text,
                phase="token",
            )
    ...
    def place_cash_order(
        ...
        if resp.status_code != 200 or js.get("rt_cd") not in (None, "0"):
            # 주문 실패 역시 KISApiError 로 래핑
            raise KISApiError(status_code=resp.status_code, body=js, phase="order")
    ...
    def get_balance(
        ...
        if resp.status_code != 200 or js.get("rt_cd") not in (None, "0"):
            raise KISApiError(status_code=resp.status_code, body=js, phase="balance")
    ...
    def get_price(self, stock_code: str, tr_id_override: Optional[str] = None) -> Dict[str, Any]:
        ...
        if resp.status_code != 200 or js.get("rt_cd") not in (None, "0"):
            raise KISApiError(status_code=resp.status_code, body=js, phase="price")
